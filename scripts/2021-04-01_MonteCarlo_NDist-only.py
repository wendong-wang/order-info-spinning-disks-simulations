...
ifLastFrameCount = 0  # 0 - using counts from all frames, 1- using counts from the last frame only
...
beta = 50 / initial_klDiv_std  # inverse of effective temperature ~ (5 / target_klDiv_NDist_std)
batchSize = 1  # how many rafts are moved together
incrementSize = 50  # unit: radius, initial increment size
finalIncrementSize = 10  # unit: radius
...
if initialPositionMethod == 1:
    ...
    raftLocations[:, currStepNum, :] = fsr.square_spiral(numOfRafts, raftRadius * 5 + 100, centerOfArena)
elif initialPositionMethod == 3:
    raftLocations[:, currStepNum, :] = fsr.hexagonal_spiral(numOfRafts, raftRadius * 5 + 200, centerOfArena)
elif initialPositionMethod == 4:
    os.chdir(dataDir)
    resultFolders = next(os.walk(dataDir))[1]
    resultFolders.sort()
    for i in range(len(resultFolders)): 
#       if resultFolders[i][-4:] =='good':
        if resultFolders[i][-4:] =='0.59': #0.63 or 0.59 or 0.58 starting psi6 value
            resultFolderID = i # the folder that contains the desired result

    resultFolderID = 95  # the folder that contains the desired result
    ...
    frameNumToLoad = -1 # 38033 for 30 rps 0.63 # 30 rps psi6 0.62 wendong pattern  # 6585 for 60 rps psi6 0.53 # 37361 for last run psi6 = 0.58 # 37003 for last run psi6 = 0.59 # 652 for 0.58 psi6 wendong's data  # the frame number to load
...
runNDist = 1  # switch for running NDist or not
runNDist_NAngles = 0
annealingSwitch = 0
...
#    ax.plot(np.arange(currStepNum), klDiv_NDist[:currStepNum], label='kldiv_NDist vs steps')
...
#    ax.legend()
##    plt.show()
#    figName = 'KL divergence of NDist_live'
#    fig.savefig(figName)
#    
#    plt.close('all')
#    
...
#    fig, ax = plt.subplots(ncols=1, nrows=1)
#    ax.plot(np.arange(currStepNum), klDiv_X[:currStepNum], label='kldiv_X vs steps')
#    ax.set_xlabel('time steps', size=20)
...
#    ax.legend()
#    plt.show()
#    figName = 'KL divergence of X_live'
#    fig.savefig(figName)
#
#    plt.close('all')
#    
...
#    fig, ax = plt.subplots(ncols=1, nrows=1)
#    ax.plot(np.arange(currStepNum), klDiv_Y[:currStepNum], label='kldiv_Y vs steps')
#    ax.set_xlabel('time steps', size=20)
...
#    ax.legend()
#    plt.show()
#    figName = 'KL divergence of Y_live'
#    fig.savefig(figName)
#    
#    plt.close('all')
...
count_NDist_all = np.zeros((len(binEdgesNeighborDistances[:-1]), 5)) # bin edges, frame with highest psi6, last frame, experimental distribution, last 10k frames
count_X_all = np.zeros((len(binEdgesX[:-1]), 5)) # bin edges, frame with highest psi6, last frame, experimental distribution, last 10k frames
count_Y_all = np.zeros((len(binEdgesY[:-1]), 5)) # bin edges, frame with highest psi6, last frame, experimental distribution, last 10k frames
count_ODist_all = np.zeros((len(binEdgesOrbitingDistances[:-1]), 5)) # bin edges, frame with highest psi6, last frame, experimental distribution, last 10k frames
...
count_ODist_all[:,4] = count_ODist[:,-10000:].sum(axis=1)


count_accepted, _ = np.histogram(np.asarray(acceptedBasedOnProbs), bins=25)

np.savetxt("NDist_hist_" + str(numOfRafts) + "Rafts_" + str(spinSpeed) + "rps_" + "LastFrame" + str(ifLastFrameCount) + "_totalAcceptedProbability" + str(count_accepted.sum()) + "_" + str(parts[0]) + "_" + str(parts[1]) + ".csv" , count_NDist_all, delimiter=",")
np.savetxt("X_hist_" + str(numOfRafts) + "Rafts_" + str(spinSpeed) + "rps_" + "LastFrame" + str(ifLastFrameCount) + "_totalAcceptedProbability" + str(count_accepted.sum()) + "_" + str(parts[0]) + "_" + str(parts[1]) + ".csv", count_X_all, delimiter=",")
np.savetxt("Y_hist_" + str(numOfRafts) + "Rafts_" + str(spinSpeed) + "rps_" + "LastFrame" + str(ifLastFrameCount) + "_totalAcceptedProbability" + str(count_accepted.sum()) + "_" + str(parts[0]) + "_" + str(parts[1]) + ".csv", count_Y_all, delimiter=",")
np.savetxt("ODist_hist_" + str(numOfRafts) + "Rafts_" + str(spinSpeed) + "rps_" + "LastFrame" + str(ifLastFrameCount) + "_totalAcceptedProbability" + str(count_accepted.sum()) + "_" + str(parts[0]) + "_" + str(parts[1]) + ".csv", count_ODist_all, delimiter=",")

#%% save the frame with maximum hex order parameter without the writing on top:
stepNumOfMaxHex = hexaticOrderParameterModuliiAvgs.argmax()
currentFrameBGR = fsr.draw_rafts_rh_coord(blankFrameBGR.copy(),
                                          np.int32(raftLocations[:, stepNumOfMaxHex, :] / scaleBar),
                                          np.int64(raftRadii / scaleBar), numOfRafts)
...
outputImageName = 'MonteCarlo_{}Rafts_numOfSteps{}_currStepNum{}_forfig.jpg'.format(
    numOfRafts, numOfTimeSteps, stepNumOfMaxHex)
cv.imwrite(outputImageName, currentFrameBGR)